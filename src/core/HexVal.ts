...
export class HexVal {
   ...
   */
  static reverseHexValues = (hex: string): string => {
   ...
   * - - - -
   * @param {HexLikeValues} hexVal A non-negative integer or a valid hex string to be processed.
   * @param {HexValueProcessorOptions} [options] `OPTIONAL` An object that changes the behavior of the convertion process.
  ...
  static processHex(hexVal: HexLikeValues, options?: HexValueProcessorOptions): string {
    switch (typeof hexVal) {
      case 'number': {
        ...
        if (options?.reversed) hex = this.reverseHexValues(hex)
   ...
   * - - - -
   * @param {HexLikeValues} hexVal - The value to convert. Must be a non-negative integer or a valid hex string.
